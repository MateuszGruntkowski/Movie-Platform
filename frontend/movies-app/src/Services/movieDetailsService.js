...
// movieDetailsService.js - zaktualizowana wersja
import api from "../api/axiosConfig";

export const movieDetailsService = {
  async getMovieDetails(movieId) {
    try {
      // Teraz wywołujemy TWOJE API zamiast TMDB bezpośrednio
  ...
  // Ta metoda może zostać - nadal może być przydatna na frontendzie
  // do formatowania URL-i obrazków które dostaniesz z backendu
  getImageUrl(imagePath, size = "w500") {
    if (!imagePath) return null;
    return `https://image.tmdb.org/t/p/${size}${imagePath}`;
  },
  ...
  // Możesz usunąć getCollectionBackdrops - to teraz obsługuje backend
};
